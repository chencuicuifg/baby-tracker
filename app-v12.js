...
function recordTime(e){return e.type==="breast"?endOf(e):e.createdAt;}
...
function render(){
  ...
  const roll=entries.filter(e=>new Date(recordTime(e)).getTime()>=cutoff);
  ...
  const feeds=entries.filter(e=>e.type==="formula"||e.type==="breast"||e.type==="pump");
  if(feeds.length){
    const latestFeed=feeds.reduce((latest,current)=>
      new Date(recordTime(current)).getTime()>new Date(recordTime(latest)).getTime()?current:latest
    );
    $("sinceFeed").textContent=elapsed(recordTime(latestFeed));
  }else{
    $("sinceFeed").textContent="No feeding logged";
  }
  renderDaily();
  const list=$("historyList");list.innerHTML="";
  const recentRecords=roll.slice().sort((a,b)=>new Date(recordTime(b))-new Date(recordTime(a)));
  recentRecords.forEach(e=>{
    const li=document.createElement("li");li.className="log-item";
    li.innerHTML=`<div><div class="log-title">${title(e)}</div><div class="log-meta">${meta(e)}</div></div><div class="log-time">${fmtTime(recordTime(e))}</div>`;
    li.onclick=()=>openEdit(e.id);list.appendChild(li);
  });
  $("emptyState").textContent="No records in the past 24 hours.";
  $("emptyState").classList.toggle("hidden",recentRecords.length>0);
